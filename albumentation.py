...
# ---------------------------------------------------------------
### 保存结果的函数
def save_prediction(image, mask_true, mask_pred, epoch, batch_idx, img_idx, save_dir="/media/disk/Backup/03drive/03SJY/second_segmentation/albumeneations/result/cotton_wool_spots/save"):
...
def get_training_augmentation():
    train_transform = [
        ...
        albu.RandomCrop(height=256, width=256, p=0.5),
        albu.GaussianBlur(p=0.2),
        albu.RandomBrightnessContrast(p=0.2),
        # 添加 Resize 操作以确保图像尺寸一致
        albu.Resize(height=256, width=256, p=1.0),  # p=1.0 表示这个操作总是被应用
    ]
...
def save_metrics_to_txt(epoch, train_logs, valid_logs, save_path='/media/disk/Backup/03drive/03SJY/second_segmentation/albumeneations/result/cotton_wool_spots/table.txt'):
...
#指标
    ...
    for epoch in range(0, 75):
        ...
        if max_score < valid_logs['iou_score']:
            ...
            torch.save(model, '/media/disk/Backup/03drive/03SJY/second_segmentation/albumeneations/model/cotton_wool_spots.pth')
            print('Model saved!')
