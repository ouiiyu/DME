...
def save_prediction(mask_pred, save_dirs, image_path):
...
if __name__ == '__main__':
    ...
    save_dirs = [
        '/media/disk/Backup/03drive/02LMW/from02drive/htr_end/attention/vessel/0',  # 0类保存目录
        '/media/disk/Backup/03drive/02LMW/from02drive/htr_end/attention/vessel/1',  # 1类保存目录
    ]
    for save_dir in save_dirs:
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
    ...
    #model = torch.load('/media/disk/Backup/03drive/03SJY/second_segmentation/albumeneations/model/vessels.pth')
    model = torch.load('/media/disk/Backup/03drive/02LMW/from02drive/vessel/2/best_model.pth')
    model = torch.load('/media/disk/Backup/03drive/03SJY/second_segmentation/no_attention/model/vessels.pth')
    model.to(DEVICE)
    model.eval()
    ...
    with torch.no_grad():
        for i, (image, image_path) in enumerate(test_loader):
            ...
            for j in range(mask_pred.shape[0]):
                save_prediction(mask_pred[j], save_dirs, image_path[j])
