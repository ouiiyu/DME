...
if __name__ == '__main__':
    ...
    save_dirs = [
        '/media/disk/Backup/03drive/02LMW/from02drive/htr_end/attention/cotton/0',  # 0类保存目录
        '/media/disk/Backup/03drive/02LMW/from02drive/htr_end/attention/cotton/1',  # 1类保存目录
    ]
    for save_dir in save_dirs:
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
    ...
    '''换.pth路径'''
    model = torch.load('/media/disk/Backup/03drive/03SJY/second_segmentation/no_attention/model/cotton_wool_spots.pth')
    model.to(DEVICE)
    model.eval()
