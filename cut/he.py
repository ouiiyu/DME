...
def save_prediction_only(mask_pred, save_dirs, image_path):
    ...
    # Determine save path based on the source directory of the image
...
if __name__ == '__main__':
    ...
    save_dirs = [
        '/media/disk/Backup/03drive/02LMW/from02drive/htr_end/attention/he/0',  # 0类保存目录
        '/media/disk/Backup/03drive/02LMW/from02drive/htr_end/attention/he/1',  # 1类保存目录
    ]
    for save_dir in save_dirs:
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
    ...
    #model = torch.load('/media/disk/Backup/03drive/02LMW/from02drive/uet/1121/best_model.pth')
    #model = torch.load('/media/disk/Backup/03drive/03SJY/second_segmentation/albumeneations/model/he.pth')
    model = torch.load('/media/disk/Backup/03drive/03SJY/second_segmentation/no_attention/model/hhe.pth')
    model.to(DEVICE)
    model.eval()
