...
if __name__ == '__main__':
    ...
    save_dirs = [
        '/media/disk/Backup/03drive/02LMW/from02drive/htr_end/attention/disc/0',  # 0类保存目录
        '/media/disk/Backup/03drive/02LMW/from02drive/htr_end/attention/disc/1',  # 1类保存目录
    ]
    for save_dir in save_dirs:
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
    ...
    #model = torch.load('/media/disk/Backup/03drive/02LMW/from02drive/disc/testend/resnet50/best_model.pth')
    model = torch.load('/media/disk/Backup/03drive/03SJY/second_segmentation/no_attention/model/disc.pth')
    model.to(DEVICE)
    model.eval()
