...
resnet50 = nn.Sequential(*list(resnet50.children())[:-1])  # 去除最后的全连接层
resnet50.eval()
...
# 初始化病灾权重
lesion_types = ["lesion_1", "lesion_2", "lesion_3", "lesion_4", "lesion_5"]
weight_range = [0.1, 1.0, 10.0]  # 扩大权重范围
area_threshold = 10  # 病灾面积最小阀值
best_accuracy = 0
best_weights = None

# 输出文件和图像保存路径
output_dir_metrics = '/media/disk/Backup/03drive/02LMW/from02drive/dme/cutmodel/kmeans/result/resnet50_/weights_metrics'
output_dir_visualizations = '/media/disk/Backup/03drive/02LMW/from02drive/dme/cutmodel/kmeans/result/resnet50_/visual'
...
for weights in itertools.product(weight_range, repeat=len(lesion_types)):
    ...
    label_ratios = [1, 1]  # 1:1 比例
    dataset_labels = [0, 1]  # 0 类和 1 类
        ...
        for image_folder in selected_folders:
            ...
            for lesion_file in os.listdir(image_folder_path):
                ...
                if lesion_mask is None:
                    print(f"警告: 无法读取病灾图像 {lesion_path}，跳过该文件。")
                    continue
                ...
                for contour in contours:
                    ...
                    if area > area_threshold:  # 仅保留面积大于阀值的区域
                ...
                if np.sum(lesion_resized) == 0:
                    print(f"警告: 病灾 {lesion_file} 的掩码为空，跳过该病灾。")
                    continue
                ...
                # 提取特征
                with torch.no_grad():
                    feature = resnet50(masked_image_tensor)
                    feature = feature.flatten().cpu().numpy()

                # 获取当前病灾类型的权重
                weight = lesion_weights.get(lesion_type, 0)
                print(f"病灾类型: {lesion_type}, 权重: {weight}")  # 打印病灾类型和权重
    ...
    with open(output_txt_path, 'w') as f:
        ...
        f.write(f"轮尺系数（Silhouette Coefficient）: {silhouette:.4f}\n")
