...
mobilenet_v2 = nn.Sequential(*list(mobilenet_v2.children())[:-1])  # 去除最后的全连接层
mobilenet_v2.eval()
...
# 输出文件和图像保存路径
output_dir_metrics = '/media/disk/Backup/03drive/02LMW/from02drive/dme/cutmodel/kmeans/result/mobile_/weights_metrics'
output_dir_visualizations = '/media/disk/Backup/03drive/02LMW/from02drive/dme/cutmodel/kmeans/result/mobile_/visual'
    ...
    for dataset_path, true_label, ratio in zip(dataset_paths, dataset_labels, label_ratios):
        ...
        for image_folder in selected_folders:
            ...
            for lesion_file in os.listdir(image_folder_path):
                ...
                # 提取特征
                with torch.no_grad():
                    feature = mobilenet_v2(masked_image_tensor)
